import React, { useEffect, useState } from 'react';
// import ProductCondition from './ProductCondition/ProductCondition';
// import VoucherDetails from './VoucherDetails/VoucherDetails';
import './ProductCondition.css';
// import UpdateProductCondition from './UpdateProductCondition/UpdateProductCondition';
import AddProductCondition from './AddProductCondition/AddProductCondition';

const ProductCondition = () => {
    const [managers, setManagers] = useState([]);
    useEffect(() => {
         fetch('https://localhost:44344/api/Pcondition/All')
         .then(res => res.json())
         .then(result => setManagers(result))
         }, [managers])
         const handleDelete=(id)=>{
              console.log(id);
              const proceed=window.confirm(`Are you sure, To delete ,id is ${id}`);
              if(proceed){
              fetch(`https://localhost:44344/api/Pcondition/delete/${id}`,{
              method: 'POST'
              })
              .then(res => res.json())
              .then(data=>{
              if(data.deletedCount>0){
              alert('Deleted Successfully');
              const remainingManagers= managers.filter(emp => emp._id !== id)
              setManagers(remainingManagers)
              }
              })
              }
              }
return (
<div className="container">
  <div className="row">
     <div className="col-lg-12">
        <div className="main-box clearfix">
           <div className="table-responsive">
              <table className="table user-list">
                 <thead>
                    <tr>
                       <th><span>Product Condition ID</span></th>
                       <th><span>Product ID</span></th>
                       <th className="text-center"><span>Product Condition</span></th>
                       {/* <th><span>Email</span></th> */}
                       {/* <th>&nbsp;</th> */}
                    </tr>
                 </thead>
                 <tbody>
                 {
                    Object.keys(managers).length !== 0 &&
                    managers.map(manager =>
                    <tr key={manager.PCId}>
                       <td>
                          <img src="" alt="" />
                          <a href="#" className="user-link">{manager.PCId}</a>
                          <span className="user-subhead"></span>
                       </td>
                       <td>
                       {manager.PId}
                       </td>
                       <td>
                     {manager.PCondition}
                       </td>
                     
                       {/* <td style={{ width: '20%' }}> */}
                       {/* <div className="table-link d-flex"> */}
                            {/* View Details  */}
                             {/* <VoucherDetails EmpID={manager.VouId}></VoucherDetails> */}
                             {/* <VoucherUpdate EmpID={manager.PCId}></VoucherUpdate> */}
                             {/* Delete Manager  */}
                             {/* <span className="fa-stack" onClick= {()=> handleDelete(manager.PCId)}> */}
                             {/* <i className="fa fa-square fa-stack-2x"></i> */}
                             {/* <i className="fa fa-trash-o fa-stack-1x fa-inverse"></i> */}
                          {/* </span> */}
                        {/* </div> */}
                            {/* </td> */}
                            </tr>
                    )}
                 </tbody>
              </table>
             <AddProductCondition></AddProductCondition>
           </div>
        </div>
     </div>
  </div>
</div>
);
};

export default ProductCondition;